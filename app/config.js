...
import { appDataDirectory, getGlobalCfgPath, lddldCfg, getWalletPath, lddlwalletCfg, getLddldRpcCert } from "./main_dev/paths";
...
export function initWalletCfg(testnet, walletPath) {
  ...
  if (!config.has("currency_display")) {
    config.set("currency_display","LDDL");
  }
...
export function getWalletCert(certPath) {
  var cert;
  certPath = getLddldRpcCert(certPath);
  try {
...
export function readLddldConfig(configPath, testnet) {
  try {
    if (!fs.existsSync(lddldCfg(configPath))) return;
    const readCfg = ini.parse(Buffer.from(fs.readFileSync(lddldCfg(configPath))).toString());
    let newCfg = {};
...
export function getLddldCert(lddldCertPath) {
  if(lddldCertPath)
    if(fs.existsSync(lddldCertPath))
      return fs.readFileSync(lddldCertPath);

  var certPath = getLddldRpcCert();
...
export function createTempLddldConf() {
  if (!fs.existsSync(lddldCfg(appDataDirectory()))) {
    ...
    fs.writeFileSync(lddldCfg(appDataDirectory()), ini.stringify(lddldConf));
  }
...
export function newWalletConfigCreation(testnet, walletPath) {
  ...
  fs.writeFileSync(lddlwalletCfg(getWalletPath(testnet, walletPath)), ini.stringify(lddlwConf));
}
