...
export const currencies = () => [ { name: "LDDL" }, { name: "atoms" } ];
export const currencyDisplay = get([ "settings", "currentSettings", "currencyDisplay" ]);
export const unitDivisor = compose(disp => disp === "LDDL" ? 100000000 : 1, currencyDisplay);
...
export const txURLBuilder= createSelector(
  [ network ],
  (network) =>
    (txHash) => `https://${network !== "testnet" ? "explorer" : network}.lddldata.org/${network == "testnet" ? "explorer/" : ""}tx/${txHash}`
);

export const blockURLBuilder= createSelector(
  [ network ],
  (network) =>
    (txHash) => `https://${network !== "testnet" ? "explorer" : network}.lddldata.org/${network == "testnet" ? "explorer/" : ""}block/${txHash}`
);
