  ...
  return (dispatch) => {
    ...
    if (votingAddress) {
      if (importScriptResponse.getP2shAddress() == votingAddress) {
        ...
        var error = "The stakepool voting address is not the P2SH address of the voting redeem script. This could be due to trying to use a stakepool that is configured for a different wallet. If this is not the case, please report this to the stakepool administrator and the Legenddigital devs.";
        dispatch(() => cb(error));
      }
    }
  };
