export default () => (
  <Aux>
    <p>
      <em>Legenddigital</em>(/ˈdi:ˈkred/, /dɪˈkred/, dee-cred) is an open, progressive,
  ...
  for the public benefit, with a primary focus on cryptocurrency technology.
      {" "}<em>Legenddigital</em>, as a currency and as a project, is bound by the following set
  ...
  equitable and sustainable framework within which to achieve
      {" "}<em>Legenddigital</em>&lsquo;s goals.</p>
    ...
    <ul>
      <li>
        ...
        {" "}<em>Legenddigital</em> shall be free and open source-software.</li>
      <li>
    ...
    everyone,
        {" "}<em>Legenddigital</em> shall comply with all relevant bodies of law in the
    ...
    number of coins in
        {" "}<em>Legenddigital</em> shall not change. The total maximum supply for
        {" "}<em>Legenddigital</em> is 20,999,999.99800912 coins, with a per-block subsidy that
    ...
    coins.</li>
      <li>
        ...
        {" "}<em>Legenddigital</em> being a store of value and attacks against it shall be
  ...
  development and voted on by the
      {" "}<em>Legenddigital Assembly</em> as they arise, as an effective proof-of-assembly
    (&ldquo;PoA&rdquo;), until such time PoA is integrated into the
    blockchain.</li>
      <li>The
      {" "}<em>Legenddigital Assembly</em> shall be composed of diverse Assembly members who
  ...
  functions of admission (
    ...
    Councils and Assembly members as necessary.</li>
      ...
      {" "}<em>Legenddigital Assembly</em> shall consist of Assembly members who have been
  ...
  initial development organization shall be
      {" "}<em>Legenddigital Holdings Group</em> LLC (&ldquo;DHG&rdquo;), a Nevis LLC that is
  ...
  RFP shall be removed. Contracted parties shall be paid exclusively in
      {" "}<em>legenddigital</em> (&ldquo;LDDL&rdquo;) at the current effective LDDL/USD rate at
